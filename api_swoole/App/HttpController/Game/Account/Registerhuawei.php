 ...
 *
 */
class Registerhuawei extends Controller
{
    protected function onRequest($action, $method) :? bool
    {
        ...
        return $this->colationPars(
            [
                ...
                'pcode' => [false, 'str', function($f){ return true;}],
                'bind' => [false, 'str', function($f){ return true;}],
            ]
        );
    }
    ...
    {
//        if (! $this->thirdparty_model->check_fb_code($this->Pars['username'], $this->Pars['id_token'])) {
//            return $this->writeJson(9010, 'token签名不对', null, true);
//        }

        $_account = $this->account_model->postRegisterYKPlayer($this->Pars, 1025, 'huawei'); //跟注册游戏的方式一样
        if (empty($_account)) {
            return $this->writeJson(9010, '注册失败', null, true);
        }
        //创建登录信息
        if (!! ($_login = $this->account_model->putPlayerLogin($_account))) {
            if(!empty($this->Pars['bind'])) {
                $userdata = [
                    'uid' => $_account['account_id'],
                    'unionid' => $this->Pars['username'],
                    'nickname' => $this->Pars['nickname'],
                    'client_uuid' => $this->Pars['clientuuid'],
                    'invit_uid' => $this->Pars['pcode'],
                ];
                $this->curl_model->faceBind($userdata);
            }
