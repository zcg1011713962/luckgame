...
class Agent_coin_up extends Controller
{
    ...
    {
        if (! $this->finance_model->postAgentCoinUp($this->Pars['username'], Helper::format_money($this->Pars['coin']), $this->Pars['ipaddr'] ?: '')) {
            return $this->writeJson(9010, '上分失败', null, true);
        }
        
        return $this->writeJson(200, '上分成功', ['username'=> $this->Pars['username'], 'msg'=> '操作成功']);
    }
}