...
class TpGameLogQueue extends AbstractProcess
{
    public function run($arg)
    {
        /* echo "<pre>";
        print_r( '这是自定义进程 TpGameLogQueue' );
        echo "</pre>".PHP_EOL; */
        
        /* $thisProcess = $this->getProcess();
        
        \EasySwoole\Component\Timer::getInstance()->after(3000, function() use(&$thisProcess) {
            echo "<pre>";
            print_r( $thisProcess );
            echo "</pre>".PHP_EOL;
                        ...
                        if (! $isRun) {
                            ...
                            while (true) {
                                $_r = $redis->evaluateSha(
                                    ...
                                    [
                                        RedisKey::LOGS_TPGPLAY,
                                        RedisKey::LOGS_TPGPLAY_HISTORY,
                                ...
                                if (isset($_r[0]) && $_r[0]) {
                                    ...
                                    Logger::getInstance()->log($_httplogstr, 'queue-tpgplay');
                                    $model->getModels()->finance_model->postPlayerTPGameLog($_r[0]);
                                }
                            }
                            $isRun = false;
                        }
