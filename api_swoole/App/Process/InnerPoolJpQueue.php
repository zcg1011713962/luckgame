...
//jp池子落地
class InnerPoolJpQueue extends AbstractProcess
{
                ...
                if (in_array($redis->ping(), ['PONG','+PONG'])) {
                    $this->addTick(500, function () use (&$isRestSystemSetting, &$isRun, &$redis) {
                        ...
                        if (! $isRun) {
                            ...
                            $sql_prefix   = 'insert into '. MysqlTables::POOL_JP. '(log_id, account_id,game_id,`type`,coin,create_time) values ';
                            $sql          = '';
                            $batch_insert = 0;
                            //取队列数据
                            while (true) {
                                $_r = $redis->evaluateSha(
                                    ...
                                    [
                                        RedisKey::LOGS_INNER_POOL_JP,
                                        RedisKey::LOGS_INNER_POOL_JP_HISTORY,
                                ...
                                if (isset($_r[0]) && $_r[0]) {
                                    ...
                                    Logger::getInstance()->log($_httplogstr, 'queue-pool-jp');
