...
//内部税落地
class InnerPoolTaxQueue extends AbstractProcess
{
            ...
            try {
                ...
                if (in_array($redis->ping(), ['PONG','+PONG'])) {
                    $this->addTick(500, function () use (&$isRestSystemSetting, &$isRun, &$redis) {
                        ...
                        if (! $isRun) {
                            $isRun = true;
                            $sql_prefix   = 'insert into '. MysqlTables::POOL_TAX. '(log_id, account_id,game_id,`type`,coin,create_time) values ';
                            $sql          = '';
                            $batch_insert = 0;
                            //取队列数据
                            while (true) {
                                $_r = $redis->evaluateSha(
                                    ...
                                    [
                                        RedisKey::LOGS_INNER_POOL_TAX,
                                        RedisKey::LOGS_INNER_POOL_TAX_HISTORY,
                                ...
                                if (isset($_r[0]) && $_r[0]) {
                                    ...
                                    Logger::getInstance()->log($_httplogstr, 'queue-pool-tax');
