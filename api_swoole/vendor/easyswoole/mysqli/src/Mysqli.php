...
class Mysqli
{
    ...
    function __construct(Config $config)
    {
        ...
        if (!$this->config->isSubQuery()) {
            ...
            $this->coroutineMysqlClient->connect($this->config->toArray());
        }
    }
    ...
    public function resetDbStatus()
    {
        if ($this->traceEnabled) {
            $this->trace[] = [$this->lastQuery, (microtime(true) - $this->traceQueryStartTime), $this->traceGetCaller()];
        }
        $this->where = [];
        $this->join = [];
        $this->orderBy = [];
    ...
    public function getList($query = "", &$counter = null, array $bindParams = [])
    {
        if ($counter !== null) {
            if (($counter = $this->getCountBySql($query)) > 0) {
    ...
    public function getCountBySql($sql = "") : int
    {
        $_sqls = [];
        $sql = strtolower($sql);
        $sql = str_replace(PHP_EOL, " ", $sql);
        $sql = preg_replace ("/\s(?=\s)/", "\\1", $sql);
        $_sqls = preg_split('/(\bselect\b|\bfrom\b|\bwhere\b|\bgroup\sby\b|\border\sby\b|\blimit\b)/i', $sql, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
        $this->removeSqlTag($_sqls, "limit", true);
        $this->removeSqlTag($_sqls, "order by", true);
        $this->replace2CountTag($_sqls);
        $lastsql = trim(implode("", $_sqls));
        $_r = $this->rawQuery($lastsql);
        $count = isset($_r) && is_array($_r) && isset($_r[0]['totalnum']) && $_r[0]['totalnum'] ? $_r[0]['totalnum'] : 0;
        
        return $count;
    }
    
    private function removeSqlTag(& $_sqls = [], $tag = "", $desc = FALSE) : void
    {
        if ($_sqls) {
            if($desc) krsort($_sqls);
            
            $_is = [];
            $_find = false;
            foreach ($_sqls as $key => $s) {
                $_s = trim($s);
                if ($_s === $tag) {
                    $_find = true;
                    $_is[] = $key;
    ...
    private function replace2CountTag(& $_sqls = []) : void
    {
        if ($_sqls) {
            $_selectI = -1;
            $_fromI = -1;
            ...
            if ($_selectI > -1 && $_fromI > -1 && $_fromI > $_selectI) {
                $_sqls[$_selectI] = "select COUNT(1) as totalnum ";
                foreach ($_is as $i) {
                    unset($_sqls[$i]);
                }
            }
