    ...
    function __run()
    {
        if (!empty($this->tasks)) {
            ...
            foreach ($this->tasks as $cronTaskName => $cronTaskClass) {
                $taskRule = $cronTaskClass::getRule();
                $next_time = CronExpression::factory($taskRule)->getNextRunDate()->getTimestamp();
                $table->set($cronTaskName, ['taskRule' => $taskRule, 'taskRunTimes' => 0, 'taskNextRunTime' => $next_time]);
            }
