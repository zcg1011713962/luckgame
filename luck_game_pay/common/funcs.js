...
module.exports = {
    ...
    rechargeChannel(probabilities = []) {
        ...
        this.log(`probabilities: ${JSON.stringify(probabilities)}, rechargeChannel: ${randomNumber}, rand: ${rand}`);
        return rand;
    },
    /**
     * url地址是否是https
    ...
    async log(text = '') {
        ...
        const timeStr = '[' + this.datestamps(nowTime, true, true, '-') + '] ';
        console.log(timeStr + text);
        try {
            text = timeStr + text + '<br />\n';
            let dateStr = this.timeWithoutLink(nowTime, false);
            //文件日志
     ...
     * @returns {string}
     */
    encryptUid (uid = 0, salt = 'zaq1!xsw2@cde3#vfr4$bgt5%nhy6^mju7&') {
        return this.encryptMd5(`${uid}_${salt}`)
    },
    /**
     * 生成订单
     * @returns {string}
     */
    genOrderNo() {
        return 'E' + moment().format('YYYYMMDDHHmmss') + this.randomNumber(100000,999999);
    },
    /**
     * 当前秒级时间戳
     * @returns {number}
