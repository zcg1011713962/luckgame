...
function UserSingleEntity:Update(record, nosync)
	if not record.uid then
		LOG_ERROR("record.uid is nil")
		return
	end
	if not self.recordset[record.uid] then
		LOG_DEBUG("recordset is nil", owner, record.uid)
	end		-- 记录不存在，返回
	-- LOG_DEBUG("UserSingleEntity:Update:", record, nosync)
...
function UserSingleEntity:SetValue(uid, field, value)
	-- LOG_DEBUG("UserSingleEntity:SetValue:", uid, field, value)
	local record = {}
	record["uid"] = uid
	if value then
		record[field] = value
