...
---@field Times table
---@field Quad table
---@field MultWeight table

---@table Config
local config = nil  --游戏配置

---@class UserRound
---@filed betcoin table
---@filed flee table
---@filed cashout table
---@filed wincoin number
---@filed tax number
...
function gamelogic.startBet(broadcast)
    ...
    for _, user in ipairs(users) do
        user.round.betcoin = {0,0}
        user.round.wincoin = 0
        user.round.flee = {0,0}
        user.round.cashout = {0,0}
    end
...
function gamelogic.startSettle()
    ...
    for _, user in ipairs(users) do
        ...
        --游戏记录
        local betcoin = user.round.betcoin[1] + user.round.betcoin[2]
        if betcoin > 0 then
            record.betGameLog(deskInfo, user, betcoin, user.round.wincoin, result, user.round.tax)
...
function gamelogic.bet(msg)
    local ret = {code = PDEFINE.RET.SUCCESS, spcode=0, c=msg.c, betcoin=msg.betcoin, flee=msg.flee, idx=msg.idx}
    ...
    if totalcoin <= 0 or (flee ~= 0 and flee < 1.01) or (idx~=1 and idx~=2) then
    ...
    if user.round.betcoin[idx] > 0 then
        ret.spcode = PDEFINE.RET.ERROR.PARAM_ILLEGAL
    ...
    user.round.betcoin[idx] = totalcoin
    user.round.flee[idx] = flee
...
function gamelogic.cashout(msg)
    ...
    if user.round.cashout[idx] > 0 then
    ...
    local betcoin = user.round.betcoin[idx]
    local wincoin = mult * betcoin
    local tax = betUtil.calcTax(betcoin, wincoin, deskInfo.taxrate)
    ...
    user.round.cashout[idx] = 1
    user.round.wincoin = user.round.wincoin + wincoin - tax
    user.round.tax = user.round.tax + tax
    user:changeCoin(PDEFINE.ALTERCOINTAG.WIN, wincoin, deskInfo)
    if user.cluster_info and wincoin > betcoin then
        user:notifyLobby(wincoin - betcoin, deskInfo.gameid, 2)
    end
    if user.cluster_info and user.istest ~= 1 then
        gamelogic.playertotalwin = gamelogic.playertotalwin + wincoin
    end

    --移除出自动逃离的队列
    user.round.flee[idx] = 0
    local i = table.findIdx(gamelogic.autofleeusers, user)
    if i > 0 and user.round.flee[3-idx] == 0 then
...
--自动取现
function gamelogic.autocashout(user, idx)
    if user.round.cashout[idx] > 0 then
        return
    end
    if agent:getState() ~= config.State.Play then
    ...
    local mult = user.round.flee[idx]
    local betcoin = user.round.betcoin[idx]
    local wincoin = mult * user.round.betcoin[idx]
    local tax = betUtil.calcTax(betcoin, wincoin, deskInfo.taxrate)
    user.round.cashout[idx] = 1
    user.round.wincoin = user.round.wincoin + wincoin - tax
    user.round.tax = tax
...
function gamelogic.update(dt)
    ...
    for i = size, 1, -1 do
        ...
        for idx = 1, 2 do --两把押注
            if user.round.flee[idx] > 0 and mult >= user.round.flee[idx] then
                local wincoin = user.round.flee[idx] * user.round.betcoin[idx]
                if not gamelogic.checkRestriction(wincoin, user.round.flee[idx]) then
                    break
                end
                gamelogic.autocashout(user, idx)
                user.round.flee[idx] = 0
                if user.round.flee[3-idx] == 0 then   --另一边也为0
                    table.remove(gamelogic.autofleeusers, i)
                end
            end
        end
...
function CMD.create(source, cluster_info, msg, ip, deskid, newplayercount, gameid)
    ...
    --加载配置
    if gameid == PDEFINE.GAME_TYPE.AVIATOR then
        config = require "crashx.aviatorconfig"
    elseif gameid == PDEFINE.GAME_TYPE.AVIATRIX then
        config = require "crashx.aviatrixconfig"
    elseif gameid == PDEFINE.GAME_TYPE.CRASHX then
        config = require "crashx.crashxconfig"
    elseif gameid == PDEFINE.GAME_TYPE.CRICKETX then
        config = require "crashx.cricketxconfig"
    elseif gameid == PDEFINE.GAME_TYPE.JETX then
        config = require "crashx.jetxconfig"
    elseif gameid == PDEFINE.GAME_TYPE.ZEPPELIN then
        config = require "crashx.zeppelinconfig"
    else
