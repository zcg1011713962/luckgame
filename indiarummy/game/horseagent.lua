...
local config = {
    ...
    -- 方位倍数
    Multiples = {1,1,1,1,1,1}, --TODO:待确认
    --筹码
...
function gamelogic.initDesk(deskInfo)
    ...
    deskInfo.round = {
        bets = {0,0,0,0,0,0},       --各位置押注总额
        chips = {{}, {}, {}, {}, {}, {}},     --各位置筹码数
        odds = {4,5,6,7,8,9},       --赔率表
    }
...
function gamelogic.startFree(broadcast)
    ...
    --生成本轮赔率
    deskInfo.round.odds = randomOdds() -- 每把都随机生成赔率
    --重置玩家数据
    ...
    for trycnt = 1, 100 do
        ...
        for _, user in ipairs(users) do
            if user.cluster_info then
                totalwin = totalwin + user.round.bets[wp] * deskInfo.round.odds[wp]
    ...
    for _, user in ipairs(users) do
        ...
        for p = 1, config.PlaceCount do
            if user.round.bets[p] > 0 then
                local wincoin = 0
                if p == wp then
                    wincoin = user.round.bets[p] * deskInfo.round.odds[wp]
                end
...
--[[
协议说明
    参考龙虎斗，开始下注时多加一个参数：odds，表示每个方位的倍率
    deskInfo:
    {
...
--通知类：
    ...
    --下注阶段
    {
        ...
        time = 18,
        odds = {3,4.1,7.6,9.3,7,10.7} --每个方法的几率
    }
    --结算阶段
    {
