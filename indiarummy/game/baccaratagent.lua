...
--计算单牌点数
function gamelogic.calcCardPoint(card)
    if baseUtil.IsAce(card) then
        return 1
    else
        local val = baseUtil.ScanValue(card)
        if val < 10 then
            return val
        end
        return 0
...
--计算手牌点数
function gamelogic.calcPoint(cards)
    local point = 0
    for _, card in ipairs(cards) do
        point = point + gamelogic.calcCardPoint(card)
    end
    return point % 10
end
--是否含有对子
function gamelogic.hasPair(cards)
    if baseUtil.ScanValue(cards[1]) == baseUtil.ScanValue(cards[2]) then --只有第一张和第二张算对子，第三张不算
        return true
    end
    return false
end
...
function gamelogic.getResult(deskInfo)
    local shufflecards = table.shuffle(config.Cards)
    local cards = table.copy(shufflecards)
    local bankercards = {}
    local playercards = {}
    for i = 1, 2 do
        table.insert(bankercards, table.remove(cards))
        table.insert(playercards, table.remove(cards))
    end

    --补牌
    local bankerpoint = gamelogic.calcPoint(bankercards)
    local playerpoint = gamelogic.calcPoint(playercards)
    if bankerpoint < 8 and playerpoint < 8 then  --庄闲任何一方点数大于8点，双方都不补牌，否则进行补牌
        --闲家小于6点补一张牌
        if playerpoint < 6 then
            local card = table.remove(cards)
            local point = gamelogic.calcCardPoint(card)
            table.insert(playercards, card)
            --如果闲家补牌，则庄家补牌规则为
            if (bankerpoint < 3)  --0，1，2点
            or (bankerpoint == 3 and point ~= 8) --3点且闲家补的8点
            or (bankerpoint == 4 and point >= 2 and point <= 7)  --4点且闲家补的2,3,4,5,6,7点
            or (bankerpoint == 5 and point >= 4 and point <= 7)  --5点且闲家4,5,6,7点
            or (bankerpoint == 6 and point >= 6 and point <= 7) --6点且闲家是6,7点
            then
                table.insert(bankercards, table.remove(cards))
            end
        else
            --如果闲家不补牌，庄家小于6点要补一张牌
            if bankerpoint < 6 then
                table.insert(bankercards, table.remove(cards))
            end
        end
    end
    --补牌后点数
    bankerpoint = gamelogic.calcPoint(bankercards)
    playerpoint = gamelogic.calcPoint(playercards)
    ...
    local result = {
        ...
        res = config.Places.Tie,  --1:庄赢 2:闲赢 3:平局
        winplace = {}  --中奖位置
    }
    if bankerpoint > playerpoint then
        result.res = config.Places.Banker
    elseif bankerpoint < playerpoint then
        result.res = config.Places.Player
    end
    table.insert(result.winplace, result.res)
    if gamelogic.hasPair(bankercards) then
        table.insert(result.winplace, config.Places.BankerPair)
    end
    if gamelogic.hasPair(playercards) then
        table.insert(result.winplace, config.Places.PlayerPair)
    end
    ...
    for trycnt = 1, 100 do
        ...
        for _, user in ipairs(users) do
            if user.cluster_info then
                ...
                --如果结果为和，下注庄和闲的注要返还
                if result.res == config.Places.Tie then
                    totalwin = totalwin + user.round.bets[config.Places.Banker]
                    totalwin = totalwin + user.round.bets[config.Places.Player]
                end
...
function gamelogic.startSettle()
    ...
    for _, user in ipairs(users) do
        ...
        for p = 1, config.PlaceCount do
            if user.round.bets[p] > 0 then
                ...
                elseif result.res == config.Places.Tie then --如果结果为和，下注庄和闲的注要返还
                    if p == config.Places.Banker or p == config.Places.Player then
                        wincoin = user.round.bets[p]
                    end
                end
    ...
    --游戏记录
    if #(deskInfo.records) >= 96 then --每当大于96条记录就清空一次
        deskInfo.records = {}
    end
    table.insert(deskInfo.records, {res=result.res, wp=result.winplace, bp=result.bankerpoint, pp=result.playerpoint})
