...
JackpotFishFeature = class(FishFeature)

function JackpotFishFeature:init(fishconfig)
    self.revenue = 0             --系统收分(烈焰风暴额外奖励从收分中出)
    self.lyfb_config = fishconfig.lyfb  --烈焰风暴配置
end

function JackpotFishFeature:endLyfb(deskInfo, userInfo)
...
function JackpotFishFeature:onUpdate(deskInfo)
...
function JackpotFishFeature:onUserFire(deskInfo, userInfo, bulletInfo)
...
function JackpotFishFeature:onUserTryCatch(deskInfo, userInfo, fishInfo, bulletInfo, rate)
    if userInfo.freeTime then
...
function JackpotFishFeature:onUserCatched(deskInfo, userInfo, fishInfo, bulletInfo)
...
function JackpotFishFeature:onUserBomb(deskInfo, userInfo, boomInfo, score)
    if userInfo.real then  --真实玩家
