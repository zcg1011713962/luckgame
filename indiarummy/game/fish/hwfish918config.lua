...
fishconfig.mults = {
    {1000,3000,6000,10000,20000,30000,40000,60000,80000,100000},
    {100000,200000,300000,400000,500000,600000,700000,800000,900000,1000000},
    {1000000,2000000,3000000,4000000,5000000,6000000,7000000,8000000,9000000,10000000},
    {10000000,14000000,18000000,22000000,26000000,30000000,34000000,38000000,44000000,50000000},
}

-- 炮台倍率
function fishconfig.getMults(gameid, ssid)
    local idx = math.min(ssid + 1, #(fishconfig.mults))
    return fishconfig.mults[idx]
end

-- AI配置
function fishconfig.getAiInfo(gameid, ssid)
    local m = fishconfig.mults
    local idx = math.min(ssid + 1, #m)
    return {
        mults = {m[idx][1], m[idx][2], m[idx][3], m[idx][4]},
        cannons = {1, 2},
    }
end
