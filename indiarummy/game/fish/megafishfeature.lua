...
MegaFishFeature = class(FishFeature)

function MegaFishFeature:init(fishconfig)
    self.revenue = 0             --系统收分(烈焰风暴额外奖励从收分中出)
    self.lyfb_config = fishconfig.lyfb  --烈焰风暴配置
end

function MegaFishFeature:endLyfb(deskInfo, userInfo)
...
function MegaFishFeature:onUpdate(deskInfo)
...
function MegaFishFeature:onUserFire(deskInfo, userInfo, bulletInfo)
...
function MegaFishFeature:onUserTryCatch(deskInfo, userInfo, fishInfo, bulletInfo, rate)
    if userInfo.freeTime then
...
function MegaFishFeature:onUserCatched(deskInfo, userInfo, fishInfo, bulletInfo)
...
function MegaFishFeature:onUserBomb(deskInfo, userInfo, boomInfo, score)
    if userInfo.real then  --真实玩家
