    ...
    for trycnt = 1, 100 do
        ...
        for _, user in ipairs(users) do
            if user.cluster_info then
                for i = 1, config.PlaceCount do
                    totalwin = totalwin + user.round.bets[i] * result.mults[i]
                end
    ...
    for _, user in ipairs(users) do
        ...
        for p = 1, config.PlaceCount do
            if user.round.bets[p] > 0 then
                local wincoin = user.round.bets[p] * result.mults[p]
...
function gamelogic.autoBet(user)
    ...
    --下注区域个数
    local placecnt = math.random(2, 3)
    ...
    local betcoin = 0
    for i = 1, placecnt do
        local placeidx = math.random(1, 6)  --6个位置随机
...
协议说明
  结算信息
    local result = {
        cards = {1,1,2,3,1,6}, --中奖牌型
        res = {3,1,1,0,0,1},   --各区域牌数量
        mults = {5,0,0,0,0,0}, --各区域中奖倍数
    }
